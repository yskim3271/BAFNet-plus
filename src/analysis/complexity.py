"""Model complexity measurement for BAFNetPlus experiments.
...
Usage:
    # All experiments
    python -m src.analysis.complexity --all \\
        --output_json results/analysis/complexity.json

    # Single experiment
    python -m src.analysis.complexity --experiment bafnetplus_50ms

    # Include RTF (requires free GPU)
    python -m src.analysis.complexity --all --measure_rtf \\
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_json",
        type=str,
        default=str(PROJECT_ROOT / "results" / "analysis" / "complexity.json"),
