...
class GRU_Group_Feature_Network(nn.Module):
    def __init__(
        self,
        in_channel: int = 64,
        hidden_size: int = 64,
        ...
        self.gru = nn.GRU(hidden_size, hidden_size, layer_num, batch_first=True, bidirectional=False)
        ...
        for i in range(time_block_num):
            time_stage.append(
                nn.Sequential(
                    ...
                    GRU_Group_Feature_Network(dense_channel, gru_hidden_size, gru_layers, causal=causal),
                )
            )
