...
from models.module_dccrn import ConvSTFT, ConviSTFT, normal_energy, ComplexConv2d
...
class bafnetplus(torch.nn.Module):
    ...
    def forward(self, tm, am, lens=False):
        ...
        est_real = est_spec[:, :self.dccrn.fft_len // 2 + 1]
        est_imag = est_spec[:, self.dccrn.fft_len // 2 + 1:]
        
        est_cspecs = torch.stack([est_real, est_imag], 1)
                        
        est_cspecs = self.postnet(est_cspecs)
        
        est_real = est_cspecs[:, 0]
        est_imag = est_cspecs[:, 1]
                
        est_spec = est_spec + torch.cat([est_real, est_imag], 1)
