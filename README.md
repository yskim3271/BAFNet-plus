...
python -m src.evaluate \
  ...
  --snr_step 0 \
  --eval_stt \
  --eval_mos                   # optional: DNSMOS + UTMOSv2 (adds ~2h / 2000 clips)
```

### Results Analysis

Paper-ready figures and summary tables are generated by two analysis scripts under `results/`:

```bash
# Gain-robustness: BCS-ACS gain-offset sweep (3 models × 3 SNR × 7 offsets)
python results/analyze_gain_robustness.py
# → results/figures/gain_robustness/*.{png,pdf}
# → results/gain_robustness_summary.csv
# → results/GAIN_ROBUSTNESS.md

# Multi-SNR: latency vs quality across SNRs (19 models × 5 SNRs)
python results/analyze_multi_snr.py
# → results/figures/multi_snr/*.{png,pdf}
# → results/multi_snr_summary.csv
# → results/MULTI_SNR.md
```

Both scripts use a paper-ready matplotlib style (serif font, grayscale-friendly markers, embedded TrueType fonts in PDF) and cover all 11–13 objective + non-intrusive + ASR metrics.
...
├── results/                   # (gitignored) experiment outputs + analysis
│   ├── experiments/           # Per-model checkpoints and configs
│   ├── eval/                  # Per-scenario evaluation JSONs
│   ├── figures/               # Paper-ready figures (gain_robustness, multi_snr)
│   ├── analyze_gain_robustness.py
│   ├── analyze_multi_snr.py
│   ├── GAIN_ROBUSTNESS.md
│   └── MULTI_SNR.md
...
Optional non-intrusive MOS metrics (with `--eval_mos` flag):
- **DNSMOS P.808**: Crowd-sourced MOS predictor (ITU-T P.808)
- **DNSMOS SIG / BAK / OVR**: Signal / background / overall quality predictors
- **UTMOSv2**: Learned MOS predictor (fusion-stage3 ensemble)
