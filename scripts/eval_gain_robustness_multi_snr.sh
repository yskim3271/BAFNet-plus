#!/bin/bash
# Evaluate calibration robustness to BCS-ACS level mismatch at multiple SNRs
...
# SNRs: -10, 10 (SNR=0 already done in previous run)
...
run_gain_eval() {
    ...
    local output_json="${EVAL_DIR}/${name}_snr${snr}_offset${offset}.json"
    local log_file="${EVAL_DIR}/${name}_snr${snr}_offset${offset}.log"
    ...
    echo "[GPU${gpu}] ${name} SNR=${snr}dB offset=${offset}dB (BCS=${bcs_gain}dB, ACS=${acs_gain}dB)"
    CUDA_VISIBLE_DEVICES=$gpu python -m src.evaluate \
        ...
        --log_file "$log_file"
    echo "[GPU${gpu}] Done: ${name} SNR=${snr}dB offset=${offset}dB -> ${output_json}"
}

# GPU0: abl_full (all SNRs x offsets sequentially)
gpu0_tasks() {
    for snr in "${SNRS[@]}"; do
        for offset in "${OFFSETS[@]}"; do
            run_gain_eval 0 abl_full "$snr" "$offset"
        done
    done
}

# GPU1: abl_no_calibration (all SNRs x offsets sequentially)
gpu1_tasks() {
    for snr in "${SNRS[@]}"; do
        for offset in "${OFFSETS[@]}"; do
            run_gain_eval 1 abl_no_calibration "$snr" "$offset"
        done
    done
}
...
echo "SNRs: ${SNRS[*]} dB | Offsets: ${OFFSETS[*]} dB"
